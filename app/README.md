...
- `src/features/build/` — the Build page: `Schedule.tsx` (the collateral schedule as a developer reads
  ...
  code shown), `Tracks.tsx` (Pyth / PreStocks / the lender agent's Meteora pool: what to call, the
  public-API `curl`, Pyth's mainnet accounts read from the browser, each track's honest limit),
  `recipes.ts` (code + run side by side, coupled by a test: `config`, `schedule`, `pyth-mainnet`,
  `marks`, `launch-status`, `solvency`, `latest-print`, `verify`, `me`, `bid-dry-run`, `subscribe`), `ProgramSurface.tsx` (from `sdk/idl/*.json`).
  The mark API (PreStocks) sends no CORS headers, so the browser reads marks from the on-chain
  `PriceCache` and shows the `curl`.
- `src/features/agent/` — the Agent page (`#/agent`): the lender agent's journey (each step computed
  from a launch record or the chain), what it does on the desk, the Meteora card, the curve set from the
  desk's numbers beside the on-chain values, and its Clawpump identity.
- `window.thewindow` — the SDK, RPC, config, console and query client for DevTools;
  `await thewindow.schedule()` returns the schedule rows the `schedule` recipe prints, and
  `await thewindow.launch()` the lender agent's pool as the `launch-status` recipe reads it.
