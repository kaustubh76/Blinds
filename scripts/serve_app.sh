...
case "${1:-status}" in
  start)
    ...
    echo "$url" > "$URL_FILE"
    for _ in $(seq 1 15); do curl -s -m 5 -o /dev/null "$url/" && break; sleep 2; done
    a="$(admin_url)"
    echo "dashboard  $url"
    [ -n "$a" ] && echo "with faucet ${url}/?admin=$a" || echo "no faucet URL yet: ./scripts/market.sh start first, then re-run status"
    ;;
  stop)
  ...
  status)
    if pgrep -f "http.server $PORT" >/dev/null && [ -f "$URL_FILE" ]; then
      ...
      echo "serving  $url ($(curl -s -m 5 -o /dev/null -w '%{http_code}' "$url/" || echo down))"
      [ -n "$a" ] && echo "faucet   ${url}/?admin=$a"
    else
      echo "not serving"
    fi
    ;;
