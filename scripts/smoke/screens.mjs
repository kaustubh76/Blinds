...
// `motion=off` keeps the animated backdrop out of every shot, so a re-take only ever differs where
// the page itself did.
...
for (const theme of ["light", "dark"]) {
  ...
  for (const [route, ms] of [
    ...
    const p = await page(1280, 1100, `${base}?theme=${theme}&motion=off#/${route}`, ms);
...
{
  const p = await page(390, 1400, `${base}?theme=light&motion=off#/`, 12_000, 217 / 390);
  await p.screenshot({ path: `${outDir}/home-phone.png` });
  await p.close();
  const m = await page(390, 900, `${base}?theme=dark&motion=off#/market`, 20_000, 217 / 390);
