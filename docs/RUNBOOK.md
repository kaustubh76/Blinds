...
| `make test-integration` fails at `setup` on a Linux x86_64 machine with `VerifyPubkeyValidity → SigmaProof(PubkeyValidity, AlgebraicRelation)` | seen on GitHub's `ubuntu-latest` runner (AMD EPYC 9V74) with Agave 4.2.1 and 4.2.2: the byte-identical validator build accepts the same proofs on macOS arm64, tier 1 (LiteSVM, same client code) passes on that runner, and the release-built service verifies the proof it sends with the SDK's own verifier before sending (`window-client` `ct.rs`). Under investigation as a platform-arithmetic difference: `cargo test -p window-client --release -- cross_platform_fixture` checks whether the machine derives the macOS escrow pubkey and accepts a macOS-made proof; `.github/workflows/tier2.yml` runs tier 2 alone on demand | run tier 2 on macOS (verified) or on devnet; report the fixture test's verdict from the failing machine |
