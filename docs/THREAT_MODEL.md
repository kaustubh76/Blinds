...
| T7 | Stale or regressing price to admit thin loans or force seizures | `PriceCache{feed_id, price, expo, publish_time, posted_slot}`; `slot − posted_slot ≤ max_price_age` at lock and seize; `publish_time` monotone. `now − publish_time ≤ max_publish_age_secs` too (A13): the posted value comes from Hermes (with a key) or Pyth's on-chain `PriceUpdateV2` account (owner- and feed-id-checked), and the feed's own `publish_time` is stored unmodified, so a quote that has stopped advancing is refused rather than disguised (A11). Under `price_source = 4` (A15) the program reads the receiver-owned account directly — owner `rec5EK…`, feed id, `Full` verification — and the keeper is out of the price path. | `attacks/attack_07_stale_price.rs`; `window-admin` price decoder test against a committed mainnet fixture |
