...
| T7 | Stale or regressing price to admit thin loans or force seizures | `PriceCache{feed_id, price, expo, publish_time, posted_slot}`; `slot − posted_slot ≤ max_price_age` at lock and seize; `publish_time` monotone. The posted value is copied from Pyth's on-chain `PriceUpdateV2` account (owner- and feed-id-checked, no API key), so a third party can diff `PriceCache` against the source account; the feed's own `publish_time` is stored unmodified, so a quote that has stopped advancing is visible rather than disguised (A11). | `attacks/attack_07_stale_price.rs`; `window-admin` price decoder test against a committed mainnet fixture |
