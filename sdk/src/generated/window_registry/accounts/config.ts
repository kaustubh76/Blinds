...
export type Config = { discriminator: ReadonlyUint8Array; admin: Address; memberCount: bigint; bump: number;  };

export type ConfigArgs = { admin: Address; memberCount: number | bigint; bump: number;  };

/** Gets the encoder for {@link ConfigArgs} account data. */
export function getConfigEncoder(): FixedSizeEncoder<ConfigArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['admin', getAddressEncoder()], ['memberCount', getU64Encoder()], ['bump', getU8Encoder()]]), (value) => ({ ...value, discriminator: CONFIG_DISCRIMINATOR }));
}

/** Gets the decoder for {@link Config} account data. */
export function getConfigDecoder(): FixedSizeDecoder<Config> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['admin', getAddressDecoder()], ['memberCount', getU64Decoder()], ['bump', getU8Decoder()]]);
}
