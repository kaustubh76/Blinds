...
export const POST_PRICE_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([130, 245, 138, 141, 114, 50, 13, 48]);
...
export type PostPriceInstructionDataArgs = { price: number | bigint; expo: number; publishTime: number | bigint;  };

export function getPostPriceInstructionDataEncoder(): FixedSizeEncoder<PostPriceInstructionDataArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['price', getU64Encoder()], ['expo', getI32Encoder()], ['publishTime', getI64Encoder()]]), (value) => ({ ...value, discriminator: POST_PRICE_DISCRIMINATOR }));
}

export function getPostPriceInstructionDataDecoder(): FixedSizeDecoder<PostPriceInstructionData> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['price', getU64Decoder()], ['expo', getI32Decoder()], ['publishTime', getI64Decoder()]]);
}

export function getPostPriceInstructionDataCodec(): FixedSizeCodec<PostPriceInstructionDataArgs, PostPriceInstructionData> {
...
expo: PostPriceInstructionDataArgs["expo"];
