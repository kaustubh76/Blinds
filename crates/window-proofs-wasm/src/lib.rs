...
#[wasm_bindgen]
pub fn bid_proofs(signature: &[u8], auditor_pubkey: &[u8], size: &str, s_min: &str) -> Result<JsValue, JsValue> {
    let sig: [u8; 64] = signature.try_into().map_err(|_| js("signature must be 64 bytes"))?;
    let kp = Keypair::from_signature(&sig).map_err(js)?;
    let auditor: [u8; 32] = auditor_pubkey.try_into().map_err(|_| js("auditor key must be 32 bytes"))?;
    let size: u64 = size.parse().map_err(js)?;
    let s_min: u64 = s_min.parse().map_err(js)?;
    let p = bid::build(&kp, &auditor, size, s_min).map_err(js)?;
    let out = BidOut { ciphertext: p.ciphertext.to_bytes().to_vec(), opening: p.opening.0.to_bytes().to_vec(), validity: bytemuck::bytes_of(&p.validity).to_vec(), range: bytemuck::bytes_of(&p.range).to_vec() };
...
pub fn lock_proofs(
    ...
    let scalars = scalar::solvency_scalars(price_cents.parse().map_err(js)?, mult_scaled.parse().map_err(js)?, haircut_bps.parse().map_err(js)?).ok_or_else(|| js("scalars"))?;
    let claim = solvency::build_collateral(&kp, &auditor, c).map_err(js)?;
    let loan_ct = GroupedCiphertext2::from_bytes(loan_ciphertext.try_into().map_err(|_| js("loan ciphertext must be 96 bytes"))?);
    let el = loan_ct.to_ciphertext(0).ok_or_else(|| js("loan ct"))?;
    let ec = claim.ciphertext.to_ciphertext(0).ok_or_else(|| js("coll ct"))?;
    let opening = Opening(solana_zk_sdk::encryption::pedersen::PedersenOpening::from_bytes(loan_opening).ok_or_else(|| js("loan opening"))?);
    let pair = solvency::build(&kp, &solvency::SolvencyInputs { collateral: &ec, c, c_opening: &claim.opening, loan: &el, l, l_opening: &opening }, &scalars).map_err(js)?;
    let out = LockOut {
...
#[wasm_bindgen]
pub fn open_note(signature: &[u8], auditor_pubkey: &[u8], note: &[u8], loan_address: &[u8]) -> Result<Vec<u8>, JsValue> {
    ...
    window_elgamal::note::open(&note, &shared, loan_address).map(|o| o.to_vec()).ok_or_else(|| js("bad note"))
}

/// Whether a 64-byte ciphertext under the member's key encrypts `expected`.
#[wasm_bindgen]
pub fn ciphertext_equals(signature: &[u8], ciphertext: &[u8], expected: &str) -> Result<bool, JsValue> {
    let sig: [u8; 64] = signature.try_into().map_err(|_| js("signature"))?;
    let kp = Keypair::from_signature(&sig).map_err(js)?;
    let ct = Ciphertext::from_bytes(ciphertext.try_into().map_err(|_| js("ciphertext must be 64 bytes"))?);
...
#[wasm_bindgen]
pub fn decrypt_small(signature: &[u8], ciphertext: &[u8], max: &str) -> Result<Option<String>, JsValue> {
    let sig: [u8; 64] = signature.try_into().map_err(|_| js("signature"))?;
    let kp = Keypair::from_signature(&sig).map_err(js)?;
    let ct = Ciphertext::from_bytes(ciphertext.try_into().map_err(|_| js("ciphertext must be 64 bytes"))?);
...
#[wasm_bindgen]
pub fn verify_print(epoch_data: &[u8], print_data: &[u8], proofs: &[u8]) -> Result<JsValue, JsValue> {
    ...
    for s in 0..2 {
        for t in 0..TICKS {
            comm[s][t].copy_from_slice(&epoch_data[off..off + 32]);
            off += 32;
        }
    }
    for s in 0..2 {
        for t in 0..TICKS {
            ...
            off += 32;
            acc[s][t] = Ciphertext { commitment: window_elgamal::Point(comm[s][t]), handle: window_elgamal::Point(h) };
        }
    }
    let mut bid_count = [[0u32; TICKS]; 2];
    for s in 0..2 {
        for t in 0..TICKS {
            bid_count[s][t] = u32::from_le_bytes(epoch_data[off..off + 4].try_into().unwrap());
            off += 4;
        }
    }
    ...
    for s in 0..2 {
        for t in 0..TICKS {
            claimed[s][t] = u64::from_le_bytes(print_data[p..p + 8].try_into().unwrap());
            p += 8;
        }
    }
    ...
    let pv = verify::PrintView { claimed_sum: claimed, r_star_tick: if status == 3 { Some(r_star) } else { None }, matched };
    let proofs: Vec<ZeroCiphertextProofData> = proofs.chunks_exact(192).map(|c| *bytemuck::from_bytes::<ZeroCiphertextProofData>(c)).collect();
    let v = verify::print(&ev, &pv, &proofs);
    #[derive(Serialize)]
    struct Out { ok: bool, nonzero: usize, proven: usize, r_star_recomputed: Option<u8>, failures: Vec<String> }
    serde_wasm_bindgen::to_value(&Out { ok: v.ok, nonzero: v.nonzero, proven: v.proven, r_star_recomputed: v.r_star_recomputed, failures: v.failures }).map_err(js)
}

// ---------------------------------------------------------------- Token-2022 confidential balance

use solana_zk_sdk::encryption::{
    auth_encryption::AeCiphertext, derivation::derive_confidential_keys_from_ikm, elgamal::ElGamalCiphertext,
};
use solana_zk_sdk_pod::encryption::auth_encryption::PodAeCiphertext;

fn token_keys(signature: &[u8]) -> Result<(solana_zk_sdk::encryption::elgamal::ElGamalKeypair, solana_zk_sdk::encryption::auth_encryption::AeKey), JsValue> {
    let sig: [u8; 64] = signature.try_into().map_err(|_| js("signature must be 64 bytes"))?;
...
pub fn token_account_keys(signature: &[u8]) -> Result<JsValue, JsValue> {
    ...
    let proof = solana_zk_sdk::zk_elgamal_proof_program::build_pubkey_validity_proof_data(&elgamal).map_err(js)?;
    #[derive(Serialize)]
    struct Out { elgamal_pubkey: Vec<u8>, pubkey_validity_proof: Vec<u8>, decryptable_zero_balance: Vec<u8> }
    let zero: PodAeCiphertext = ae.encrypt(0).into();
    serde_wasm_bindgen::to_value(&Out { elgamal_pubkey: elgamal.pubkey().to_bytes().to_vec(), pubkey_validity_proof: bytemuck::bytes_of(&proof).to_vec(), decryptable_zero_balance: zero.0.to_vec() }).map_err(js)
}
...
#[wasm_bindgen]
pub fn confidential_balances(signature: &[u8], decryptable: &[u8], pending_lo: &[u8], pending_hi: &[u8]) -> Result<JsValue, JsValue> {
    let (elgamal, ae) = token_keys(signature)?;
    let ae_ct = AeCiphertext::from_bytes(decryptable).ok_or_else(|| js("decryptable"))?;
    let available = ae.decrypt(&ae_ct).ok_or_else(|| js("cannot decrypt available balance with this key"))?;
    ...
    #[derive(Serialize)]
    struct Out { available: String, pending: String }
    serde_wasm_bindgen::to_value(&Out { available: available.to_string(), pending: (lo + (hi << 16)).to_string() }).map_err(js)
}
...
#[wasm_bindgen]
pub fn transfer_proofs(signature: &[u8], available_ct: &[u8], decryptable: &[u8], amount: &str, destination_elgamal_pubkey: &[u8], auditor_pubkey: &[u8]) -> Result<JsValue, JsValue> {
    let (elgamal, ae) = token_keys(signature)?;
    let available = ElGamalCiphertext::from_bytes(available_ct).ok_or_else(|| js("available ct"))?;
    let ae_ct = AeCiphertext::from_bytes(decryptable).ok_or_else(|| js("decryptable"))?;
    let amount: u64 = amount.parse().map_err(js)?;
    let dest = window_elgamal::keys::pubkey_from_bytes(destination_elgamal_pubkey.try_into().map_err(|_| js("dest key"))?).map_err(js)?;
    let auditor = window_elgamal::keys::pubkey_from_bytes(auditor_pubkey.try_into().map_err(|_| js("auditor key"))?).map_err(js)?;
    let p = spl_token_confidential_transfer_proof_generation::transfer::transfer_split_proof_data(&available, &ae_ct, amount, &elgamal, &ae, &dest, Some(&auditor)).map_err(js)?;
    let current = ae.decrypt(&ae_ct).ok_or_else(|| js("balance"))?;
    let new_balance: PodAeCiphertext = ae.encrypt(current.checked_sub(amount).ok_or_else(|| js("insufficient"))?).into();
    #[derive(Serialize)]
    struct Out { equality: Vec<u8>, validity: Vec<u8>, range: Vec<u8>, auditor_lo: Vec<u8>, auditor_hi: Vec<u8>, new_decryptable: Vec<u8> }
    serde_wasm_bindgen::to_value(&Out {
        equality: bytemuck::bytes_of(&p.equality_proof_data).to_vec(),
        validity: bytemuck::bytes_of(&p.ciphertext_validity_proof_data_with_ciphertext.proof_data).to_vec(),
...
#[wasm_bindgen]
pub fn withdraw_proofs(signature: &[u8], available_ct: &[u8], decryptable: &[u8], amount: &str) -> Result<JsValue, JsValue> {
    let (elgamal, ae) = token_keys(signature)?;
    let available = ElGamalCiphertext::from_bytes(available_ct).ok_or_else(|| js("available ct"))?;
    let ae_ct = AeCiphertext::from_bytes(decryptable).ok_or_else(|| js("decryptable"))?;
    let current = ae.decrypt(&ae_ct).ok_or_else(|| js("balance"))?;
    let amount: u64 = amount.parse().map_err(js)?;
    let p = spl_token_confidential_transfer_proof_generation::withdraw::withdraw_proof_data(&available, current, amount, &elgamal).map_err(js)?;
    let new_balance: PodAeCiphertext = ae.encrypt(current.checked_sub(amount).ok_or_else(|| js("insufficient"))?).into();
    #[derive(Serialize)]
    struct Out { equality: Vec<u8>, range: Vec<u8>, new_decryptable: Vec<u8> }
    serde_wasm_bindgen::to_value(&Out { equality: bytemuck::bytes_of(&p.equality_proof_data).to_vec(), range: bytemuck::bytes_of(&p.range_proof_data).to_vec(), new_decryptable: new_balance.0.to_vec() }).map_err(js)
}
