...
use window_elgamal::{
    decrypt, encrypt, keys::Keypair, solvency_delta, Ciphertext, GroupedCiphertext2, Point, BASEPOINT,
};
...
fn grouped_layout_matches_sdk_and_handle_order() {
    ...
    // handle 1 is the auditor's: decrypting with the auditor key recovers 77·G
...
fn accumulation_is_homomorphic_and_residual_encrypts_zero() {
    ...
    let (a, _) = encrypt::grouped2(&member.pubkey_bytes(), &auditor.pubkey_bytes(), 1_000_000_000).unwrap();
    let (b, _) = encrypt::grouped2(&member.pubkey_bytes(), &auditor.pubkey_bytes(), 250_000_000).unwrap();
    ...
    // the member's own handle decrypts nothing useful about the aggregate (different key)
    assert!(!decrypt::equals(&member, &acc, 1_250_000_000));
}

#[test]
fn solvency_delta_matches_direct_dalek_computation() {
    let borrower = Keypair::random();
    let (c, l) = (1_000_000u64, 200_000_000_000u64); // 1,000.000 shares, 200,000 USDC
    ...
    let plain = k_c * c - k_l * l;
    assert!(decrypt::equals(&borrower, &delta, plain));
}
